use uuid::Uuid;
use diesel::{RunQueryDsl, QueryDsl, ExpressionMethods, OptionalExtension, JoinOnDsl};
...
impl IConversationRepository for BaseRepo {
    ...
    async fn find_by_user(&self, user_id: Uuid) -> diesel::QueryResult<Vec<Conversation>> {
        self.run(move |conn| {
            conversations::table
                .filter(conversations::id.eq_any(
    ...
    async fn find_existing_direct_conversation(&self, user1_id: Uuid, user2_id: Uuid) -> diesel::QueryResult<Option<Conversation>> {
        self.run(move |conn| {
