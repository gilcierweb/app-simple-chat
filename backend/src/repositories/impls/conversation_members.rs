...
impl IConversationMemberRepository for BaseRepo {
    async fn create(&self, item: &NewConversationMember) -> diesel::QueryResult<ConversationMember> {
        let item = item.clone();
        self.run(move |conn| {
    ...
    async fn find_by_conversation(&self, conversation_id: Uuid) -> diesel::QueryResult<Vec<ConversationMember>> {
    ...
    async fn find(&self, conversation_id: Uuid, user_id: Uuid) -> diesel::QueryResult<Option<ConversationMember>> {
        use diesel::{QueryDsl, ExpressionMethods};
        self.run(move |conn| {
            conversation_members::table
                .filter(conversation_members::conversation_id.eq(conversation_id))
