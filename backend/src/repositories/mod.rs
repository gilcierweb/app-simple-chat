...
impl IUserRepository for BaseRepo {
    ...
    async fn find(&self, id: &Uuid) -> diesel::QueryResult<User> {
        ...
        let id = *id;
        self.run(move |conn| users::table.find(id).first::<User>(conn)).await
    }
    ...
    async fn destroy(&self, id: &Uuid) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| diesel::delete(users::table.find(id)).execute(conn)).await
    }

    async fn find_by_username_or_email(&self, username_or_email: &str) -> diesel::QueryResult<Option<User>> {
    ...
    async fn update_password(&self, id: &Uuid, encrypted_password: &str) -> diesel::QueryResult<usize> {
    ...
    async fn record_failed_login(&self, user_id: &Uuid, _max_attempts: i32) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl, ExpressionMethods};
        use crate::db::schema::users::dsl;
        let id = *user_id;
        self.run(move |conn| {
    ...
    async fn record_successful_login(&self, user_id: &Uuid, ip: Option<IpNet>) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl, ExpressionMethods};
        use crate::db::schema::users::dsl;
        let id = *user_id;
    ...
    async fn create_password_reset_token(&self, user_id: &Uuid, token: &str, sent_at: NaiveDateTime) -> diesel::QueryResult<usize> {
        self.update_reset_token(user_id, Some(token.to_string()), Some(sent_at)).await
    }
    ...
    async fn enable_2fa(&self, user_id: &Uuid, _backup_codes: &[String]) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl, ExpressionMethods};
        use crate::db::schema::users::dsl;
        let id = *user_id;
        self.run(move |conn| {
...
impl IProfileRepository for BaseRepo {
    ...
    async fn find(&self, id: &Uuid) -> diesel::QueryResult<Profile> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| profiles::table.find(id).first::<Profile>(conn)).await
    }
    ...
    async fn destroy(&self, id: &Uuid) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| diesel::delete(profiles::table.find(id)).execute(conn)).await
    }
...
impl IRefreshTokenRepository for BaseRepo {
    async fn all(&self) -> diesel::QueryResult<Vec<RefreshToken>> {
        use diesel::RunQueryDsl;
        self.run(|conn| refresh_tokens::table.load::<RefreshToken>(conn)).await
    }

    async fn find(&self, id: &Uuid) -> diesel::QueryResult<RefreshToken> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| refresh_tokens::table.find(id).first::<RefreshToken>(conn)).await
    }
    ...
    async fn destroy(&self, id: &Uuid) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| diesel::delete(refresh_tokens::table.find(id)).execute(conn)).await
    }

    async fn find_by_token_hash(&self, token_hash: &str) -> diesel::QueryResult<Option<RefreshToken>> {
        use diesel::{QueryDsl, RunQueryDsl, OptionalExtension};
...
impl IAuditLogRepository for BaseRepo {
    async fn all(&self) -> diesel::QueryResult<Vec<AuditLog>> {
        use diesel::RunQueryDsl;
        self.run(|conn| audit_logs::table.load::<AuditLog>(conn)).await
    }

    async fn find(&self, id: &Uuid) -> diesel::QueryResult<AuditLog> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| audit_logs::table.find(id).first::<AuditLog>(conn)).await
    }
    ...
    async fn destroy(&self, id: &Uuid) -> diesel::QueryResult<usize> {
        use diesel::{QueryDsl, RunQueryDsl};
        let id = *id;
        self.run(move |conn| diesel::delete(audit_logs::table.find(id)).execute(conn)).await
    }
